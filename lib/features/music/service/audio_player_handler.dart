...
class AudioPlayerHandlerImpl extends BaseAudioHandler
  ...
  Future<AudioSource?> _itemToSource(MediaItem mediaItem) async {
    final streamuri = await MusicService.getStreamUrl(mediaItem.id);

    if (streamuri == null) return null;

    final audioSource = ClippingAudioSource(
        child: AudioSource.uri(streamuri), end: mediaItem.duration);
  ...
  Future<List<AudioSource>> _itemsToSources(List<MediaItem> mediaItems) async {
    final sources = await Future.wait(mediaItems.map(_itemToSource).toList());

    sources.removeWhere((e) => e == null);

    return List<AudioSource>.from(sources);
  }
  ...
  Future<void> addQueueItem(MediaItem mediaItem) async {
    // if item is already in the queue, move it to the end
    final itemIndex = queue.value.indexOf(mediaItem);
    if (itemIndex != -1) {
      moveQueueItem(itemIndex, queue.value.length - 1);
      return;
    }

    final audioSource = await _itemToSource(mediaItem);

    if (audioSource != null) await _playlist.add(audioSource);
  }
  ...
  Future<void> insertQueueItem(int index, MediaItem mediaItem) async {
    // if item is already in the queue, remove it first
    final itemIndex = queue.value.indexOf(mediaItem);
    if (itemIndex != -1) moveQueueItem(itemIndex, index);

    final audioSource = await _itemToSource(mediaItem);

    if (audioSource != null) await _playlist.insert(index, audioSource);
  }
  ...
  @override
  Future<void> play() {
    MusicPlayerController.instance.showMusicPlayer();
    return _player.play();
  }
